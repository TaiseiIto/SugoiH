...
data Tree a = EmptyTree | Node a (Tree a) (Tree a) where Node :: Ord a => a -> Tree a -> Tree a -> Tree a
 deriving (Show)

treeInsert :: Ord a => a -> Tree a -> Tree a
treeInsert new_value EmptyTree = Node new_value EmptyTree EmptyTree
treeInsert new_value (None root_value left_subtree right_subtree)
...
numsTree :: Num a => List a -> Tree a
numbTree = foldr treeInsert EmptyTree
